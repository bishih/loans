...
def validateJwt(jwt):
    resp = requests.get('https://ttlogin.azurewebsites.net/api/v1/validate', headers={'x-jwt-assertion': jwt})
...
class LoansEP(MethodResource, Resource):
    ...
    def get(self, bank_id, account_id):
        is_validate = validateJwt(request.headers.get('x-jwt-assertion'))
        if not is_validate:
            return 'JWT not legal'
        loans = []
        for loan in data:
            print(loan)
